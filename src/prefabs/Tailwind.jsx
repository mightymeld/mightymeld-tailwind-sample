  ...
  return (
    <div className="bg-white rounded-xl w-96 p-8 flex flex-col justify-betweenh-96">
      <div className="">
        ...
        <div className="text-4xl font-bold mt-6"> $0 <span className="font-light text-sm">/ monthly</span> </div>
      ...
      <button className="bg-green-700 w-full text-white rounded-xl p-3 hover:opacity-90" >Start Trial</button>
    </div>
  )
}
...
export function Accordion() {
  return (
    <details className="list-none bg-white " >
      ...
      <p className=" p-3 rounded-xl" >Lorem ipsum dolor sit amet consectetur adipisicing elit. Sapiente est commodi neque officia dolorem veritatis quas voluptas repellat nisi porro ipsa nulla, unde, nobis enim nesciunt optio exercitationem blanditiis error.</p>
    </details>
  )
}
...
export function FLoating_Label() {
  return (
    <div className="relative">
      <input type="text" id="floating_label" className="block rounded-xl px-2.5 pb-2.5 pt-5 w-full text-sm text-gray-900 bg-white border-0 appearance-none focus:outline-none focus:ring-0 focus:border-gray-600 peer" placeholder=" " />
      <label htmlFor="floating_label" className="absolute text-sm text-slate-600 duration-300 transform -translate-y-4 scale-75 top-4 z-10 origin-[0] left-2.5 peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-4">Label</label>
    </div>
  )
}
...
export function Search() {
  return (
    <div className="relative">
      ...
      <input type="search" id="default-search" className="block w-full p-4 pl-10 text-sm text-gray-900 border border-gray-300 rounded-lg bg-gray-50 focus:ring-0 focus:border-0 " placeholder="Search " />
    </div>
  )
}
  ...
  return (
    <div className="p-2">
      <div className="flex gap-2 border-b">
        <div onClick={showTabOne} className={`cursor-pointer py-2 border-b-2 ${tab === 1 ? "border-green-700" : "border-transparent"}`}>Tab One</div>
        <div onClick={showTabTwo} className={`cursor-pointer py-2 border-b-2 ${tab === 2 ? "border-green-700" : "border-transparent"}`}>Tab Two</div>
      </div>
      <div className="py-2">
...
export function TextArea() {
  return <textarea id="message" rows={4} className="block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500 " placeholder="Write your thoughts here..."></textarea>
}